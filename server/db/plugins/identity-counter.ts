...
const counterSchema = new Schema({
    _model: {type: String, require: true},
...
export default function autoIncrementPlugin(schema, options) {
    ...
    // If we don't have reference to the counterSchema or the IdentityCounter _model then the plugin was most likely not
    ...
    var settings = {
            _model: null, // The _model to configure the plugin for.
    ...
    switch (typeof(options)) {
        // If string, the user chose to pass in just the _model name.
        case 'string':
            settings._model = options;
            break;
    ...
    if (settings._model == null)
        throw new Error("_model must be set");
    ...
    // Find the counter for this _model and the relevant field.
    IdentityCounter.findOne(
        {_model: settings._model, field: settings.field},
        function (err, counter) {
            if (!counter) {
                // If no counter exists then create one and save it.
                counter = new IdentityCounter({
                    _model: settings._model,
    ...
    // Declare a function to get the next counter for the _model/schema.
            ...
            (function save() {
                ...
                if (ready) {
                    ...
                    if (typeof doc[settings.field] === 'number') {
                        IdentityCounter.findOneAndUpdate(
                            // IdentityCounter documents are identified by the _model and field that the plugin was invoked for.
                            // Check also that count is less than field value.
                            {_model: settings._model, field: settings.field, count: {$lt: doc[settings.field]}},
                        ...
                        // Find the counter collection entry for this _model and field and update it.
                        IdentityCounter.findOneAndUpdate(
                            // IdentityCounter documents are identified by the _model and field that the plugin was invoked for.
                            {_model: settings._model, field: settings.field},
